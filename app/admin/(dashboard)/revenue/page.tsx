...
export default function RevenuePage() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      // Filter ONLY delivered orders for the Revenue page
      const deliveredOrders = (data || []).filter((o: Order) => o.status === "Delivered");
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="bg-primary/10 text-primary font-bold text-2xl px-6 py-3 rounded-xl border border-primary/20">
          ₹{totalRevenue}
        </div>
      </div>
      ...
      {orders.length === 0 ? (
        <div className="flex flex-col items-center justify-center bg-card border border-border rounded-xl p-12 text-center shadow-sm">
          ...
          <h3 className="text-lg font-semibold mb-1">No Delivered Orders Yet</h3>
          <p className="text-muted-foreground">Once orders are marked as 'Delivered', they will appear here and count towards revenue.</p>
        </div>
      ) : (
                ...
                {orders.map((order) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className="text-xs font-semibold rounded-full px-3 py-1 border outline-none bg-green-100 text-green-800 border-green-200">
                        {order.status}
                      </span>
                    </td>
